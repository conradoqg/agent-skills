...
- **Capability + triggers, not the workflow.** State *what* it does and *when*
  to reach for it — but do NOT encode the step-by-step procedure in the
  description. Tested pitfall: when the description summarizes the workflow, the
  agent follows that summary as a shortcut and never reads the body (obra). Keep
  the "how" in the body; the description only gets the agent to open it.

...
This skill decides **whether and when** to harvest. It does not own the craft of
writing a good `SKILL.md`. **REQUIRED SUB-SKILL: `authoring-skills`** — the
shared rubric, template, and per-tool reference for *how* to write (and review) a
skill. Step 5 hands the write off to it.
...
> - First read the `authoring-skills` skill (a sibling skill in the same skills
>   directory): its `references/skill-authoring.md` and
>   `assets/SKILL.template.md`. Author `SKILL.md` to that spec, plus any
>   `references/` or `assets/` files the procedure warrants.
...
- **Capture procedures, not answers.** "Join orders to customers for EMEA" is
  ...
  skill. See the `authoring-skills` skill.
- **Keep `SKILL.md` tight** (< 500 lines, < ~5000 tokens). Push detail into
  `references/` and tell the reader *when* to load each file.

For the full authoring spec, template, and per-tool reference, use the
**`authoring-skills`** skill (sibling in the same skills directory):
`authoring-skills/references/skill-authoring.md`,
`authoring-skills/assets/SKILL.template.md`, and
`authoring-skills/references/agents.md`.
